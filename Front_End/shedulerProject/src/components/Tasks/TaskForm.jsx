...
const TaskForm = ({ task, onClose, onSuccess }) => {
  const [formData, setFormData] = useState({
    ...
    creation_datetime: task?.creation_datetime || new Date().toISOString().slice(0, 16),
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Creation Date & Time</label>
            <input
              type="datetime-local"
              name="creation_datetime"
              value={formData.creation_datetime}
              onChange={handleChange}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
            />
          </div>
